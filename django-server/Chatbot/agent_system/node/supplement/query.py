...
def search_nonpersonal_sup(state: AgentState) -> Dict[str, Any]:
    """
    ...
    if step == 1:
        node_messages.append("search_nonpersonal_sup 노드: '사용자의 입력에서 추출된 모든 조건이 적용된 영양제 상품 검색을 성공했습니다.'")
    elif step == 2:
        node_messages.append("search_nonpersonal_sup 노드: '모든 검색 조건으로는 상품을 찾을 수 없었습니다. 사용자의 입력에서 추출된 조건에서 '영양소', '영양제유형', '건강목적' 만으로 영양제 상품 검색을 성공했습니다.'")
    elif step == 3:
        node_messages.append("search_nonpersonal_sup 노드: '모든 검색 조건으로는 상품을 찾을 수 없었습니다. 사용자의 입력에서 추출된 조건에서 '영양소', '건강목적' 만으로 영양제 상품 검색을 성공했습니다.'")
    elif step == 4:
        node_messages.append("search_nonpersonal_sup 노드: '모든 검색 조건으로는 상품을 찾을 수 없었습니다. 사용자의 입력에서 추출된 조건에서 '건강목적' 만으로 영양제 상품 검색을 성공했습니다.'")
    
    return {
        "kag_results": all_results[:20],
        "personalized_info": personalized_info,
        "node_messages": node_messages
    }


def search_personal_sup(state: AgentState) -> Dict[str, Any]:
    """
    extracted_info를 바탕으로 그래프 DB Cypher 쿼리를 생성하고 실행하는 노드
    결과가 20개 미만일 경우 단계적으로 필터링을 제거하며 추가 결과를 가져옵니다.
    """
    ...
    if user.gender is not None:
        if user.gender == "male":
            ...
            gender = "여자"
        node_messages.append(f"search_personal_sup 노드: '사용자의 계정 정보에서 성별 정보 '{gender}'을 찾았습니다.'")
    elif user_health_info and user_health_info.get("gender"):
        if user_health_info.get("gender") == "남성":
            gender = "남자"
        else:
            gender = "여자"
        node_messages.append(f"search_personal_sup 노드: '사용자의 설문조사 정보에서 성별 정보 '{gender}'을 찾았습니다.'")
    else:
        gender = "남자"
        node_messages.append("search_personal_sup 노드: '사용자의 계정, 설문조사 정보에서 성별 정보를 찾을 수 없어 기본값인 '남자'가 성별로 설정되었습니다.'")

    # 사용자 나이 계산
    today = date.today()
    if user.birth_date is not None:
        age = today.year - user.birth_date.year - ((today.month, today.day) < (user.birth_date.month, user.birth_date.day))
        node_messages.append(f"search_personal_sup 노드: '사용자의 계정 정보에서 나이 정보 '{age}'을 찾았습니다.'")
    else:
        ...
        if user_health_info and user_health_info.get("age"):
            ...
            if age_str == "19-29세":
                age = 20
                node_messages.append(f"search_personal_sup 노드: '사용자의 설문조사 정보에서 나이 정보 '{age}'을 찾았습니다.'")
            elif age_str == "30-49세":
                age = 40
                node_messages.append(f"search_personal_sup 노드: '사용자의 설문조사 정보에서 나이 정보 '{age}'을 찾았습니다.'")
            elif age_str == "50-64세":
                ...
                age = 25  # 기본값
                node_messages.append(f"search_personal_sup 노드: '사용자의 계정, 설문조사 정보에서 나이 정보를 찾을 수 없어 기본값인 '{age}'가 나이로 설정되었습니다.'")
        else:
            age = 25  # 기본값
            node_messages.append(f"search_personal_sup 노드: '사용자의 계정, 설문조사 정보에서 나이 정보를 찾을 수 없어 기본값인 '{age}'가 나이로 설정되었습니다.'")
    print(f"사용자 나이: {age}")
    ...
    if extracted_info.get("purpose_tag"):
        node_messages.append("search_personal_sup 노드: '목적 태그 기반 맞춤형 영양제 검색을 시작합니다.'")
        # 우선순위 영양소 리스트
        ...
        for tag in extracted_info["purpose_tag"]:
            ...
            for nutrient in ordered_nutrients:
                ...
                if recent_intake < daily_amount:
                    ...
                    node_messages.append(f"search_personal_sup 노드: '{nutrient} 영양소가 {deficiency_percentage:.1f}% 부족하여 해당 영양소를 포함한 영양제를 검색했습니다.'")
    ...
    # 영양소별 처리
    elif extracted_info.get("nutrients"):
        node_messages.append("search_personal_sup 노드: '사용자가 직접 언급한 영양소 기반 맞춤형 영양제 검색을 시작합니다.'")
        ...
        for nutrient in extracted_info["nutrients"]:
            ...
            if recent_intake < daily_amount:
                ...
                node_messages.append(f"search_personal_sup 노드: '{nutrient} 영양소가 {excess_percentage:.1f}% 과다 섭취 중이어 관련 영양소를 검색했습니다.'")
                ...
                for related in related_nutrients:
                    ...
                    if related_intake < related_daily_amount:
                        ...
                        node_messages.append(f"search_personal_sup 노드: '{related_nutrient} 관련 영양소가 {deficiency_percentage:.1f}% 부족하여 해당 영양소를 포함한 영양제를 검색했습니다.'")
    ...
    # 영양제 유형별 처리.
    elif extracted_info.get("supplement_types"):
        node_messages.append("search_personal_sup 노드: '영양제 유형 기반 맞춤형 영양제 검색을 시작합니다.'")
    ...
    node_messages.append(f"search_personal_sup 노드: '맞춤형 영양제 검색이 완료되었습니다.'")
